...
import { resolveApiBaseUrl, resolveApiHttpUrl } from "../apiUrlResolver";
import type { Session, Message, Part, Provider, Config, Model, Agent, TextPartInput, FilePartInput, Event } from "@kronoscode-ai/sdk/v2";
import type { PermissionRequest } from "@/types/permission";
import type { QuestionRequest } from "@/types/question";
import { JsonRpcGatewayClient, type GatewayEvent } from "./json-rpc-gateway";
...
const TRANSIENT_LOG_DEDUPE_WINDOW_MS = 15000;
const transientLogTimestamps = new Map<string, number>();

const shouldEmitTransientLog = (key: string): boolean => {
  const now = Date.now();
  const previous = transientLogTimestamps.get(key) ?? 0;
  if (now - previous < TRANSIENT_LOG_DEDUPE_WINDOW_MS) {
    return false;
  }
  transientLogTimestamps.set(key, now);
  return true;
};
...
const ensureAbsoluteBaseUrl = (candidate: string): string => {
  ...
  if (
    ...
    (protocol === "moz-extension:" || protocol === "chrome-extension:" || protocol === "safari-web-extension:")
  ) {
    return normalized;
  }
...
const resolveDesktopBaseUrl = (): string | null => {
  ...
  const embeddedBase = resolveApiBaseUrl();
  if (ABSOLUTE_URL_PATTERN.test(embeddedBase)) {
    return embeddedBase;
  }
  ...
  const apiPrefix =
    typeof desktopServer.apiPrefix === "string" && desktopServer.apiPrefix.trim().length > 0 ? desktopServer.apiPrefix.trim() : "/api";

  try {
    return new URL(apiPrefix, origin).toString().replace(/\/+$/, "");
  } catch {
    return `${origin}/api`;
  }
};
...
export type BrowserRuntimeProvider = "desktop-native" | "kronoscode-relay" | "zen-host" | "extension";
...
export type BrowserRuntimeState = {
  ...
  backend: "embedded-browseros-bridge" | "chrome-cdp" | "embedded-fallback" | "playwright" | string;
...
export type ToolReliabilityEnvelope = {
...
export type SessionStatusEnvelope = {
  ...
  runHealth?: SessionRunHealth;
};

export type ResumeLastObjectiveSnapshot = {
  session_resume_token: string;
  session: Session;
  objective?: {
    message_id?: string;
    text?: string;
  ...
  pending_todos: Array<{ content: string; status: string; priority: string }>;
  recent_tool_context: Array<{
    tool: string;
...
export type WorkflowRunNodeState = {
  ...
  reliability?: ToolReliabilityEnvelope;
};
...
export type WorkflowRunState = {
  ...
  session_id: string;
  workflow_id: string;
  playbook_id: "plan_todo_task_bootstrap" | "todo_execution_cycle" | "review_wrapup";
  workflow_stage: string;
...
export type WorkflowPlaybook = {
  id: "plan_todo_task_bootstrap" | "todo_execution_cycle" | "review_wrapup";
  name: string;
  description: string;
  nodes: Array<{
    nodeID: string;
...
class OpencodeService {
  ...
  private gatewayConnectPromise: Promise<JsonRpcGatewayClient> | null = null;
  private gatewayCursor = 0;
  private gatewayServerInstanceId: string | undefined;
  private gatewaySupported: boolean | undefined;

  constructor(baseUrl: string = DEFAULT_BASE_URL) {
    const desktopBase = resolveDesktopBaseUrl();
    const requestedBaseUrl = desktopBase || resolveApiBaseUrl() || baseUrl;
    this.baseUrl = ensureAbsoluteBaseUrl(requestedBaseUrl);
    this.client = createOpencodeClient({ baseUrl: this.baseUrl });
    void this.connectGateway().catch(() => undefined);
  }

  private getSurfaceId(): string | undefined {
    if (typeof window === "undefined") return undefined;
    return new URLSearchParams(window.location.search).get("surfaceId") ?? undefined;
  }

  private gatewayEventToRouted(event: GatewayEvent): RoutedOpencodeEvent {
    return {
  ...
  private async connectGateway(): Promise<JsonRpcGatewayClient> {
    if (this.gateway?.connectionState === "open") return this.gateway;
    if (this.gatewayConnectPromise) return this.gatewayConnectPromise;
    this.gatewayConnectPromise = (async () => {
      const ticketResponse = await fetch(`${this.baseUrl.replace(/\/+$/, "")}/kronoscode/gateway-ticket`, {
        method: "POST",
      ...
      if (ticketResponse.status === 404 || ticketResponse.status === 405) {
        this.gatewaySupported = false;
        throw new Error("gateway_unsupported");
      }
      if (!ticketResponse.ok) {
        throw new Error(`gateway ticket failed with status ${ticketResponse.status}`);
      }
      const ticket = (await ticketResponse.json()) as { wsUrl?: string };
      if (!ticket.wsUrl) {
        this.gatewaySupported = false;
        throw new Error("gateway_unsupported");
      }
      ...
      gateway.onEvent((event) => {
        ...
        this.emitGlobalSseEvent(this.gatewayEventToRouted(event));
      });
      await gateway.connect(ticket.wsUrl);
      const initialized = await gateway.request<{
      ...
      this.gatewaySupported = true;
      this.gateway = gateway;
      return gateway;
    })();
    try {
      return await this.gatewayConnectPromise;
    } finally {
      this.gatewayConnectPromise = null;
    }
  }

  private async requestGateway<T>(method: string, params: Record<string, unknown>): Promise<{ handled: boolean; value?: T }> {
    if (this.gatewaySupported === false) return { handled: false };
    try {
      const gateway = await this.connectGateway();
      return { handled: true, value: await gateway.request<T>(method, params) };
    } catch (error) {
      if (error instanceof Error && error.message === "gateway_unsupported") {
      ...
      try {
        ...
        if (retryError instanceof Error && retryError.message === "gateway_unsupported") {
          return { handled: false };
        }
        throw retryError;
      }
  ...
  async getSystemInfo(): Promise<{ homeDirectory: string; username?: string }> {
    ...
    try {
      const response = await this.client.path.get(this.currentDirectory ? { directory: this.currentDirectory } : undefined);
    ...
    if (!candidates.size) {
      try {
        const project = await this.client.project.current(this.currentDirectory ? { directory: this.currentDirectory } : undefined);
  ...
  async listSessions(): Promise<Session[]> {
    const response = await this.client.session.list(this.currentDirectory ? { directory: this.currentDirectory } : undefined);
    return Array.isArray(response.data) ? response.data : [];
  }

  async createSession(params?: { parentID?: string; title?: string }): Promise<Session> {
    const gateway = await this.requestGateway<Session>("session.create", {
      directory: this.currentDirectory,
      parentID: params?.parentID,
      title: params?.title,
    });
    if (gateway.handled && gateway.value) return gateway.value;
  ...
  async getSessionMessages(id: string, limit?: number, before?: string): Promise<{ info: Message; parts: Part[] }[]> {
    const base = this.baseUrl.replace(/\/+$/, "");
    const url = new URL(`${base}/session/${encodeURIComponent(id)}/message`);
    if (this.currentDirectory) url.searchParams.set("directory", this.currentDirectory);
    if (typeof limit === "number") url.searchParams.set("limit", String(limit));
    if (before) url.searchParams.set("before", before);
    const response = await fetch(url);
    if (!response.ok) throw new Error(`Failed to load session messages (${response.status})`);
    return (await response.json()) as { info: Message; parts: Part[] }[];
  }
  ...
  async resumeLastObjective(options?: { directory?: string | null }): Promise<ResumeLastObjectiveSnapshot | null> {
    try {
      const base = this.baseUrl.replace(/\/$/, "");
      const url = new URL(`${base}/session/resume_last_objective`);
      const directory = options?.directory ?? this.currentDirectory ?? null;
      ...
      const response = await fetch(url.toString(), {
        ...
        body: JSON.stringify({
          ...(typeof directory === "string" && directory.trim().length > 0 ? { directory: directory.trim() } : {}),
        }),
      });
      ...
      return payload as ResumeLastObjectiveSnapshot;
    } catch {
      return null;
    }
  }

  async listWorkflowPlaybooks(): Promise<WorkflowPlaybook[]> {
    try {
      const base = this.baseUrl.replace(/\/$/, "");
      const url = new URL(`${base}/workflow/playbook`);
      if (this.currentDirectory && this.currentDirectory.length > 0) {
        url.searchParams.set("directory", this.currentDirectory);
      }
      const response = await fetch(url.toString(), {
      ...
      const payload = await response.json().catch(() => null);
      return Array.isArray(payload) ? (payload as WorkflowPlaybook[]) : [];
    } catch {
      return [];
    }
  }

  async listWorkflowRuns(options?: { sessionID?: string; limit?: number }): Promise<WorkflowRunState[]> {
    try {
      const base = this.baseUrl.replace(/\/$/, "");
      const url = new URL(`${base}/workflow/run`);
      if (this.currentDirectory && this.currentDirectory.length > 0) {
        url.searchParams.set("directory", this.currentDirectory);
      }
      ...
      if (!response.ok) return [];
      const payload = await response.json().catch(() => null);
      return Array.isArray(payload) ? (payload as WorkflowRunState[]) : [];
    } catch {
      return [];
    }
  }

  async listActiveWorkflowRuns(options?: { sessionID?: string }): Promise<
    Array<{
      workflow_run_id: string;
      session_id: string;
      playbook_id: "plan_todo_task_bootstrap" | "todo_execution_cycle" | "review_wrapup";
  ...
  > {
    try {
      const base = this.baseUrl.replace(/\/$/, "");
      const url = new URL(`${base}/workflow/run/active`);
      if (this.currentDirectory && this.currentDirectory.length > 0) {
        url.searchParams.set("directory", this.currentDirectory);
      }
      if (options?.sessionID) {
  ...
  async getWorkflowRun(runID: string): Promise<WorkflowRunState | null> {
    try {
      const base = this.baseUrl.replace(/\/$/, "");
      const url = new URL(`${base}/workflow/run/${encodeURIComponent(runID)}`);
      if (this.currentDirectory && this.currentDirectory.length > 0) {
        url.searchParams.set("directory", this.currentDirectory);
      }
      const response = await fetch(url.toString(), {
      ...
      return payload as WorkflowRunState;
    } catch {
      return null;
    }
  }
  ...
  async startWorkflowRun(input: {
    sessionID: string;
    playbookID: "plan_todo_task_bootstrap" | "todo_execution_cycle" | "review_wrapup";
  ...
  async cancelWorkflowRun(runID: string): Promise<WorkflowRunState | null> {
    try {
      const base = this.baseUrl.replace(/\/$/, "");
      const url = new URL(`${base}/workflow/run/${encodeURIComponent(runID)}/cancel`);
      if (this.currentDirectory && this.currentDirectory.length > 0) {
        url.searchParams.set("directory", this.currentDirectory);
      }
      const response = await fetch(url.toString(), {
  ...
  async sendMessage(params: {
    ...
    // This ID is both the optimistic UI identity and the backend idempotency key.
    const baseTimestamp = Date.now();
    const tempMessageId = params.messageId ?? `msg_${baseTimestamp.toString(16)}${Math.random().toString(36).slice(2, 16)}`;
    ...
    const gateway = await this.requestGateway<{ accepted: boolean; messageID: string }>("prompt.submit", prompt);
    if (gateway.handled) return gateway.value?.messageID ?? tempMessageId;
  ...
  async abortSession(id: string): Promise<boolean> {
    const gateway = await this.requestGateway<boolean>("session.abort", {
      directory: this.currentDirectory,
      session_id: id,
    });
    if (gateway.handled) return gateway.value === true;
  ...
  async getSessionStatus(): Promise<Record<string, SessionStatusEnvelope>> {
    return this.getSessionStatusForDirectory(this.currentDirectory ?? null);
  }

  async getSessionStatusForDirectory(directory: string | null | undefined): Promise<Record<string, SessionStatusEnvelope>> {
    try {
      const base = this.baseUrl.replace(/\/$/, "");
      ...
      return data as Record<string, SessionStatusEnvelope>;
    } catch {
      return {};
    }
  }

  async getGlobalSessionStatus(): Promise<Record<string, SessionStatusEnvelope>> {
  ...
  async getWebServerSessionActivity(): Promise<Record<string, { type: string }> | null> {
    try {
      const response = await fetch(resolveApiHttpUrl("/api/session-activity"), {
        method: "GET",
        headers: {
          Accept: "application/json",
        },
  ...
  async listToolIds(options?: { directory?: string | null }): Promise<string[]> {
    try {
      const directory =
        typeof options?.directory === "string" ? options.directory.trim() : this.currentDirectory ? this.currentDirectory.trim() : "";
  ...
  async getBrowserRuntimeState(sessionID: string): Promise<BrowserRuntimeState> {
    ...
    throw new Error(payload && typeof payload.error === "string" ? payload.error : `Failed to fetch browser state (${response.status})`);
  }
    ...
    if (!response.ok || !payload || typeof (payload as BrowserFrameResponse).base64 !== "string") {
      const payloadError =
        payload && typeof payload === "object" && "error" in payload && typeof (payload as { error?: unknown }).error === "string"
          ...
          : null;
      throw new Error(payloadError ?? `Failed to fetch browser frame (${response.status})`);
    }
  ...
  subscribeToBrowserEvents(
    ...
    const endpoint =
      typeof window !== "undefined"
        ? resolveApiHttpUrl(`/api/runtime/browser/events?sessionID=${encodeURIComponent(sessionID)}`)
        : (() => {
            const base = this.baseUrl.replace(/\/$/, "");
            const url = new URL(`${base}/experimental/browser/events`);
            url.searchParams.set("sessionID", sessionID);
            return url;
          })();
  ...
  async replyToPermission(requestId: string, reply: "once" | "always" | "reject", options?: { message?: string }): Promise<boolean> {
    const gateway = await this.requestGateway<boolean>("permission.reply", {
      directory: this.currentDirectory,
  ...
  async replyToQuestion(requestId: string, answers: string[] | string[][]): Promise<boolean> {
    ...
    const gateway = await this.requestGateway<boolean>("question.reply", {
      directory: this.currentDirectory,
      request_id: requestId,
  ...
  async rejectQuestion(requestId: string): Promise<boolean> {
    const gateway = await this.requestGateway<boolean>("question.reject", {
      directory: this.currentDirectory,
      request_id: requestId,
    });
    if (gateway.handled) return gateway.value === true;
  ...
  async getProviders(): Promise<{
    ...
    const response = await this.client.config.providers(this.currentDirectory ? { directory: this.currentDirectory } : undefined);
  ...
  async listAgents(): Promise<Agent[]> {
    try {
      const response = await this.client.app.agents(this.currentDirectory ? { directory: this.currentDirectory } : undefined);
      return response.data || [];
    } catch {
      return [];
    }
  ...
  private normalizeRoutedSsePayload(raw: unknown): RoutedOpencodeEvent | null {
    ...
    const normalizedDirectory =
      typeof directoryCandidate === "string" ? (this.normalizeCandidatePath(directoryCandidate) ?? directoryCandidate.trim()) : null;
  ...
  private ensureGlobalSseStarted() {
    ...
    this.globalSseTask = this.runGlobalTransportLoop(abortController)
      .catch((error) => {
        if ((error as Error)?.name === "AbortError" || abortController.signal.aborted) {
          return;
        }
  ...
  private getGlobalSseCoalesceKey(event: RoutedOpencodeEvent): string | null {
    ...
    const properties =
      typeof payload.properties === "object" && payload.properties !== null ? (payload.properties as Record<string, unknown>) : null;
  ...
  private async runGlobalSseLoop(abortController: AbortController): Promise<void> {
    ...
    while (!abortController.signal.aborted) {
      try {
        ...
        if (shouldEmitTransientLog("global-sse-stream-error")) {
          console.info("[OpencodeClient] Global SSE stream transient error (will retry).");
        }
  ...
  private async runGlobalTransportLoop(abortController: AbortController): Promise<void> {
    const reconnectDelays = [1_000, 2_000, 4_000, 8_000, 15_000];
    let failures = 0;
    while (!abortController.signal.aborted && this.gatewaySupported !== false) {
      try {
        const gateway = await this.connectGateway();
        this.globalSseIsConnected = true;
        failures = 0;
        ...
        this.notifyGlobalSseError(error);
        if (failures >= 6) {
          this.notifyGlobalSseError(new Error("KronosCode gateway recovery requires attention."));
        }
      }
      if (abortController.signal.aborted) return;
      this.gateway?.close();
      this.gateway = null;
      const delay = reconnectDelays[Math.min(Math.max(0, failures - 1), reconnectDelays.length - 1)];
      await new Promise((resolve) => setTimeout(resolve, delay));
    }
    if (!abortController.signal.aborted) {
  ...
  subscribeToEvents(
    ...
    (async () => {
      const resolvedDirectory =
        typeof directoryOverride === "string" && directoryOverride.trim().length > 0 ? directoryOverride.trim() : this.currentDirectory;
  ...
  async readFile(path: string): Promise<string> {
    ...
    if (!response.ok) {
      ...
      throw new Error(message || `Failed to read file: ${response.status} ${response.statusText}`);
    }
  ...
  async listFiles(directory?: string): Promise<Record<string, unknown>[]> {
    ...
    if (!response.ok) {
      const message = await response.text().catch(() => "");
      throw new Error(message || `Failed to list files: ${response.status} ${response.statusText}`);
    }

    const data = await response.json();
    if (!Array.isArray(data)) {
      throw new Error("File list response was not an array");
    }
    return data;
  }
  ...
  async listCommands(): Promise<Array<{ name: string; description?: string; agent?: string; model?: string }>> {
    try {
      const response = await this.client.command.list(this.currentDirectory ? { directory: this.currentDirectory } : undefined);
  ...
  async listCommandsWithDetails(): Promise<
    Array<{ name: string; description?: string; agent?: string; model?: string; template?: string; subtask?: boolean }>
  > {
    try {
      const response = await this.client.command.list(this.currentDirectory ? { directory: this.currentDirectory } : undefined);
  ...
  async createDirectory(dirPath: string, options?: { allowOutsideWorkspace?: boolean }): Promise<{ success: boolean; path: string }> {
  ...
  async listLocalDirectory(directoryPath: string | null | undefined, options?: { respectGitignore?: boolean }): Promise<FilesystemEntry[]> {
    ...
    if (desktopFiles) {
      try {
        ...
        } else if (shouldEmitTransientLog("list-directory-aborted-desktop")) {
          console.debug("Directory listing aborted while request was being replaced.");
        }
        throw error;
      }
    }
    ...
    try {
      ...
      if (!isAbortLikeError(error)) {
        console.error("Failed to list directory contents:", error);
      } else if (shouldEmitTransientLog("list-directory-aborted-http")) {
        console.debug("Directory listing aborted while request was being replaced.");
      }
      throw error;
    }
  ...
  async searchFiles(
    ...
    const desktopFiles = getDesktopFilesApi();
    const directory =
      typeof options?.directory === "string" && options.directory.trim().length > 0 ? options.directory.trim() : this.currentDirectory;
  ...
  async getFilesystemHome(): Promise<string | null> {
    ...
    try {
      ...
      if (!response.ok) {
        const error = await response.json().catch(() => ({}));
        const message = typeof error.error === "string" && error.error.length > 0 ? error.error : "Failed to resolve home directory";
        throw new Error(message);
      }
  ...
  async setKronosCodeWorkingDirectory(directoryPath: string | null | undefined): Promise<DirectorySwitchResult | null> {
    ...
    try {
      ...
      if (!response.ok) {
        const error = payload ?? {};
        const message =
          typeof error.error === "string" && error.error.length > 0 ? error.error : "Failed to update KronosCode working directory";
        throw new Error(message);
      }
