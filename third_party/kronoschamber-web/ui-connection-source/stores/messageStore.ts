...
import { useUIStore, type MainTab } from "./useUIStore";
...
type RuntimeStatusSnapshot = {
    ...
    connectors?: Record<string, RuntimeConnectorStatus>;
    tasks?: { running?: number };
    streamHealth?: { ready?: boolean; restarting?: boolean; lastError?: string | null };
    openCodeRunning?: boolean;
    workspaces?: Record<string, { available?: boolean; reason?: string; requiresDesktopRuntime?: boolean }>;
};
...
const APP_CAPABILITY_GUIDE: Record<MainTab, string> = {
    landing: "Product overview and landing experience.",
    chat: "Conversation, coding help, and agent/tool execution.",
    design: "Embedded Krondesign workspace backed by the active KronosCode runtime.",
    plan: "Task planning and execution sequencing.",
    creative: "Legacy creative alias. Route to Canvas.",
    ops: "Native Chamber ops workspace for collections, forms, records, actions, workflows, and managed agents.",
    social: "Native Chamber social workspace for campaigns, drafts, approvals, scheduling, and agent-assisted publishing.",
    video: "Native Chamber video workspace for clip jobs, review queues, and video-to-social handoff.",
    runtime: "Canonical Chamber runtime for the primary browser, desktop interaction, background runs, artifacts, and logs.",
    workflow: "Node-based workflow design, execution runs, templates, and step-level inspection.",
    canvas: "Native Chamber canvas for notes, browser blocks, terminal blocks, artifacts, and AI branch trees.",
    git: "Repository status, branches, commits, and PR-oriented operations.",
    diff: "File change review and integration support.",
    terminal: "Project shell execution and command output inspection.",
    files: "Workspace file browsing, selection, and editing context.",
    marketplace: "Integration and MCP marketplace management.",
    browser: "Legacy browser alias. Route to Runtime.",
    business: "Legacy business alias. Route to Ops.",
    "desktop-control": "Legacy desktop-control alias. Route to Runtime.",
    sandbox:
        "Isolated E2B/QEMU sandbox session management — create, start, pause, stop, takeover, and monitor agent execution environments.",
    "desktop-sessions":
        "Desktop session orchestration — manage remote desktop sessions, host switching, and human-in-the-loop takeover flows.",
};

const RUNTIME_STATUS_CACHE_TTL_MS = 30_000;
let runtimeStatusCache: { expiresAt: number; value: RuntimeStatusSnapshot | null } = {
    expiresAt: 0,
    value: null,
};
let runtimeCapabilitiesCache: { expiresAt: number; value: RuntimeCapabilitySnapshot | null } = {
    expiresAt: 0,
    value: null,
};
...
const readRuntimeStatusSnapshot = async (): Promise<RuntimeStatusSnapshot | null> => {
    const now = Date.now();
    if (runtimeStatusCache.expiresAt > now) {
    ...
    try {
        ...
        if (!response.ok) {
            runtimeStatusCache = { expiresAt: now + 8_000, value: runtimeStatusCache.value };
            return runtimeStatusCache.value;
        }
        const payload = (await response.json().catch(() => null)) as RuntimeStatusSnapshot | null;
        runtimeStatusCache = { expiresAt: now + RUNTIME_STATUS_CACHE_TTL_MS, value: payload };
        return payload;
    } catch {
        runtimeStatusCache = { expiresAt: now + 8_000, value: runtimeStatusCache.value };
        return runtimeStatusCache.value;
...
const readRuntimeCapabilitiesSnapshot = async (): Promise<RuntimeCapabilitySnapshot | null> => {
    const now = Date.now();
    if (runtimeCapabilitiesCache.expiresAt > now) {
    ...
    try {
        ...
        if (!response.ok) {
            runtimeCapabilitiesCache = { expiresAt: now + 8_000, value: runtimeCapabilitiesCache.value };
            return runtimeCapabilitiesCache.value;
        }
        const payload = (await response.json().catch(() => null)) as RuntimeCapabilitySnapshot | null;
        runtimeCapabilitiesCache = { expiresAt: now + RUNTIME_STATUS_CACHE_TTL_MS, value: payload };
        return payload;
    } catch {
        runtimeCapabilitiesCache = { expiresAt: now + 8_000, value: runtimeCapabilitiesCache.value };
        return runtimeCapabilitiesCache.value;
...
const summarizeConnectorStatus = (name: string, status: RuntimeConnectorStatus | undefined): string => {
    if (!status) {
        return `${name}=unknown`;
    }
    const state = status.available === false ? "unavailable" : status.available === true ? "available" : "unknown";
    const health = typeof status.health === "string" && status.health.length > 0 ? status.health : "unknown";
    const provider = typeof status.provider === "string" && status.provider.length > 0 ? status.provider : "n/a";
    return `${name}(${state}, health=${health}, provider=${provider})`;
};
...
const isDesktopRuntimeAvailable = (): boolean => {
    ...
    return runtimeApis?.runtime?.isDesktop === true;
};

const resolveWorkspaceAvailability = (
    tab: MainTab,
    runtimeStatus: RuntimeStatusSnapshot | null,
): { available: boolean; reason?: string } => {
    const status = runtimeStatus?.workspaces?.[tab];
    if (status && typeof status.available === "boolean") {
        if (status.requiresDesktopRuntime === true && !isDesktopRuntimeAvailable()) {
            return { available: false, reason: "desktop runtime required" };
        }
        return {
            available: status.available,
            reason: typeof status.reason === "string" && status.reason.length > 0 ? status.reason : undefined,
        };
    }

    if (tab === "runtime" || tab === "browser" || tab === "desktop-control") {
        return isDesktopRuntimeAvailable() ? { available: true } : { available: false, reason: "desktop runtime required" };
    }

    if (tab === "creative") {
        return resolveWorkspaceAvailability("canvas", runtimeStatus);
    }

    return { available: true };
};

const buildCapabilityPreface = async (_sessionId: string, inputMode: "normal" | "shell"): Promise<string | undefined> => {
    if (inputMode !== "normal") {
        return undefined;
    }

    const activeMainTab = useUIStore.getState().activeMainTab;
    const runtimeStatus = await readRuntimeStatusSnapshot();
    const runtimeCapabilities = await readRuntimeCapabilitiesSnapshot();

    const connectorSummary = runtimeStatus?.connectors
        ? Object.entries(runtimeStatus.connectors)
              .sort(([a], [b]) => a.localeCompare(b))
              .map(([name, status]) => summarizeConnectorStatus(name, status))
              .join(", ")
        : "none";
    const availableModes = Array.isArray(runtimeStatus?.availableModes) ? runtimeStatus!.availableModes.join(", ") : "unknown";
    const streamHealth = runtimeStatus?.streamHealth
        ? `ready=${runtimeStatus.streamHealth.ready === true}, restarting=${runtimeStatus.streamHealth.restarting === true}`
        : "unknown";
    const runningTasks =
        typeof runtimeStatus?.tasks?.running === "number" && Number.isFinite(runtimeStatus.tasks.running) ? runtimeStatus.tasks.running : 0;

    const activeTabCapability = APP_CAPABILITY_GUIDE[activeMainTab] ?? "Unknown tab capability.";
    const capabilitySummary = runtimeCapabilities?.summary
        ? `total=${runtimeCapabilities.summary.total ?? "unknown"}, available=${runtimeCapabilities.summary.available ?? "unknown"}, degraded=${runtimeCapabilities.summary.degraded ?? "unknown"}, unavailable=${runtimeCapabilities.summary.unavailable ?? "unknown"}, not_implemented=${runtimeCapabilities.summary.notImplemented ?? "unknown"}`
        : "unknown";
    const availableCapabilityHighlights = (runtimeCapabilities?.capabilities ?? [])
        .filter((capability) => capability?.status === "available")
        .slice(0, 10)
        .map((capability) => {
            const id = typeof capability.id === "string" ? capability.id : "unknown";
            const provider = typeof capability.provider === "string" ? capability.provider : "n/a";
            const actions = Array.isArray(capability.actions) ? capability.actions.slice(0, 5).join("/") : "";
            return `${id}[${provider}${actions ? `:${actions}` : ""}]`;
        })
        .join(", ");
    const workspaceCapabilities = (Object.keys(APP_CAPABILITY_GUIDE) as MainTab[]).map((tab) => {
        const availability = resolveWorkspaceAvailability(tab, runtimeStatus);
        const availabilityLabel = availability.available
            ? "available"
            : `unavailable${availability.reason ? ` (${availability.reason})` : ""}`;
        return `${tab}: ${APP_CAPABILITY_GUIDE[tab]} [${availabilityLabel}]`;
    });
    const activeTabAvailability = resolveWorkspaceAvailability(activeMainTab, runtimeStatus);

    return [
        "[KronosChamber capability context]",
        `Active workspace tab: ${activeMainTab}`,
        `Active tab capability: ${activeTabCapability}`,
        `Active tab availability: ${activeTabAvailability.available ? "available" : "unavailable"}${activeTabAvailability.reason ? ` (${activeTabAvailability.reason})` : ""}`,
        `Workspace capabilities: ${workspaceCapabilities.join(" | ")}`,
        `Runtime mode: ${runtimeStatus?.mode ?? "unknown"} (available modes: ${availableModes})`,
        `Runtime connectors: ${connectorSummary}`,
        `Runtime stream health: ${streamHealth}; running background tasks: ${runningTasks}`,
        `Runtime capability registry: ${capabilitySummary}`,
        `Available capability highlights: ${availableCapabilityHighlights || "none reported"}`,
        `Policy: choose tools/workflows based on the available connectors and tab capabilities above; if unavailable, state the limitation and fallback.`,
    ].join("\n");
};
        ...
        persist(
            (set, get) => ({
                ...
                loadMessages: async (sessionId: string, limit?: number) => {
                    ...
                    const targetLimit =
                        typeof limit === "number" && Number.isFinite(limit) ? limit : Math.max(baseLimit, userExpandedLimit ?? 0);
                    ...
                    const allMessages = await executeWithSessionDirectory(sessionId, () =>
                    ...
                    const hasMoreAbove = typeof fetchLimit === "number" ? messagesWithoutReverted.length > targetLimit : false;
                    ...
                    set((state) => {
                        ...
                        const previousMessagesById = new Map(
                            previousMessages.filter((msg) => typeof msg.info?.id === "string").map((msg) => [msg.info.id as string, msg]),
                        );
                        ...
                        const normalizedMessages = messagesToKeep.map((message) => {
                            ...
                            const existingEntry = infoWithMarker?.id ? previousMessagesById.get(infoWithMarker.id as string) : undefined;
                        ...
                        try {
                            ...
                            try {
                                ...
                                if (shellPayload || slashShellPayload) {
                                    ...
                                    const capabilityPreface = await buildCapabilityPreface(sessionId, inputMode);
                                    await kronoscodeClient.sendMessage({
                                        id: sessionId,
                                        providerID,
                                        modelID,
                                        text: content,
                                        prefaceText: capabilityPreface,
                                        prefaceTextSynthetic: true,
                                        agent,
                                        variant,
                                        files: filePayloads.length > 0 ? filePayloads : undefined,
                                        additionalParts:
                                            additionalPartsPayload && additionalPartsPayload.length > 0
                                                ? additionalPartsPayload
                                                : undefined,
                    ...
                    set((state) => {
                        ...
                        const finalizeAbortState = (result: Partial<MessageState>): Partial<MessageState> => {
                            const shouldClearAbortFlag =
                                (actualRole === "assistant" || actualRole === "user") && state.sessionAbortFlags.has(sessionId);
                        ...
                        if (actualRole === "assistant" && messageIndex !== -1) {
                            ...
                            const nextStreamingMap = setStreamingIdForSession(
                                updates.streamingMessageIds ?? state.streamingMessageIds,
                                sessionId,
                                messageId,
                            );
                        ...
                        if (messageIndex === -1) {
                            if (actualRole === "user") {
                                ...
                                (window as any).__messageTracker?.(
                                    messageId,
                                    `new_user_part_type:${(normalizedPart as any).type || "unknown"}`,
                                );
                                ...
                                const agentMode =
                                    typeof sessionAgent === "string" && sessionAgent.trim().length > 0 ? sessionAgent.trim() : undefined;
                                ...
                                if (pendingMeta) {
                                    updates.pendingUserMessageMetaBySession = cleanupPendingUserMessageMeta(
                            ...
                            if ((part as any)?.type === "text") {
                                ...
                                if (textIncoming.length > 0) {
                                    const latestUser = [...messagesArray].reverse().find((m) => m.info.role === "user");
                                    if (latestUser) {
                            ...
                            const contextStore = useContextStore.getState();
                            const sessionAgent =
                                contextStore.getSessionAgentSelection(sessionId) ?? contextStore.getCurrentAgent(sessionId);
                            const agentMode =
                                typeof sessionAgent === "string" && sessionAgent.trim().length > 0 ? sessionAgent.trim() : undefined;
                ...
                forceCompleteMessage: (
                    ...
                    messageId: string,
                    source: "timeout" | "cooldown" = "timeout",
                ) => {
                ...
                updateMessageInfo: (sessionId: string, messageId: string, messageInfo: any) => {
                    set((state) => {
                        ...
                        if (messageIndex === -1) {
                            ...
                            if (normalizedSessionMessages.length > 0) {
                                ...
                                const incomingCreated =
                                    typeof incomingInfoToCompare?.time?.created === "number" ? incomingInfoToCompare.time.created : null;
                            ...
                            if (incomingInfo && incomingInfo.role === "user") {
                                ...
                                const updates: Partial<MessageState> = {
                                    ...
                                    ...(pendingMeta
                                        ? {
                                              pendingUserMessageMetaBySession: cleanupPendingUserMessageMeta(
                                                  state.pendingUserMessageMetaBySession,
                                                  sessionId,
                                              ),
                                          }
                        ...
                        const isUserMessage =
                            existingInfo.userMessageMarker === true || existingInfo.clientRole === "user" || existingInfo.role === "user";
                ...
                loadMoreMessages: async (sessionId: string, direction: "up" | "down" = "up") => {
                    ...
                    const memLimits = getMemoryLimits();
                    const baseLimit =
                        memoryState.historyLimit ??
                        Math.max(memLimits.HISTORICAL_MESSAGES + memLimits.FETCH_BUFFER, currentMessages.length + memLimits.FETCH_BUFFER);
                    ...
                    try {
                        ...
                        const before = firstCurrentMessage.info.id;
                        const fetchLimit = memLimits.HISTORY_CHUNK + 1;
                        const allMessages = await executeWithSessionDirectory(sessionId, () =>
                            kronoscodeClient.getSessionMessages(sessionId, fetchLimit, before),
                        );
                        const hasMoreAbove = allMessages.length > memLimits.HISTORY_CHUNK;
                        const newMessages = dedupeMessagesById(allMessages).slice(-memLimits.HISTORY_CHUNK);
                        ...
                        set((snapshot) => {
                            ...
                            const mergedMessages = dedupeMessagesById([...newMessages, ...latestCurrent]);
                            ...
                            newMemoryState.set(sessionId, {
                                ...
                                historyLimit: desiredLimit,
                                historyComplete: !hasMoreAbove,
                                totalAvailableMessages: Math.max(latestMemory.totalAvailableMessages ?? 0, mergedMessages.length),
                            });
