    ...
    devtools(
        (set, get) => ({
            ...
            setVoiceMode: (mode: import("./types/sessionTypes").VoiceMode) => {
                ...
                appendSessionActivityEvent: (sessionId, event) => {
                    set((state) => {
                        const next = new Map(state.sessionActivityEvents);
                        const current = next.get(sessionId) ?? [];
                        const deduped = current.filter((entry) => entry.id !== event.id || entry.type !== event.type);
                        next.set(sessionId, [...deduped, event].slice(-200));
                        return { sessionActivityEvents: next };
                    });
                },
