...
const MAX_NEW_FILE_DIFF_STAT_ESTIMATES = 200;
const TRANSIENT_DIFF_STAT_ERROR_CODES = new Set(['ENOENT', 'ENOTDIR']);
const GIT_STATUS_WARNING_THROTTLE_MS = 60_000;
const STATUS_CACHE_TTL_MS = 1500;
const gitStatusWarningTimestamps = new Map();
...
const shouldLogGitStatusWarning = (key) => {
  const now = Date.now();
  const previous = gitStatusWarningTimestamps.get(key);
  if (typeof previous === 'number' && now - previous < GIT_STATUS_WARNING_THROTTLE_MS) {
    return false;
  }
  gitStatusWarningTimestamps.set(key, now);
  return true;
};
...
export async function getStatus(directory) {
  const directoryPath = normalizeDirectoryPath(directory);
  const cacheKey = typeof directoryPath === 'string' && directoryPath.trim() ? directoryPath : '.';
  ...
  const loadPromise = (async () => {
    ...
    try {
      // Expanding untracked directories can turn a workspace status refresh into a multi-million-file scan.
      ...
      const limitedStatusFiles =
        statusFiles.length > MAX_STATUS_FILES ? statusFiles.slice(0, MAX_STATUS_FILES) : statusFiles;

      if (
        statusFiles.length > limitedStatusFiles.length &&
        shouldLogGitStatusWarning(`status-truncate:${directoryPath}:${statusFiles.length}`)
      ) {
        console.warn(
          `[git] status for ${directoryPath} returned ${statusFiles.length} files; truncating to ${MAX_STATUS_FILES} to avoid UI overload.`,
        );
      }
      ...
      const newFileCandidates = limitedStatusFiles.filter((file) => {
      ...
      const filesForDiffStatEstimation =
        newFileCandidates.length > MAX_NEW_FILE_DIFF_STAT_ESTIMATES
          ? newFileCandidates.slice(0, MAX_NEW_FILE_DIFF_STAT_ESTIMATES)
          : newFileCandidates;

      if (
        newFileCandidates.length > filesForDiffStatEstimation.length &&
        shouldLogGitStatusWarning(`new-file-skip:${directoryPath}:${newFileCandidates.length}`)
      ) {
        console.warn(
          `[git] skipping diff stat estimation for ${
            newFileCandidates.length - filesForDiffStatEstimation.length
          } new files in ${directoryPath} to keep status responsive.`,
        );
      }

      const newFileStats = await Promise.all(
        filesForDiffStatEstimation.map(async (file) => {
          const existing = diffStats[file.path];
          ...
          try {
            ...
            if (TRANSIENT_DIFF_STAT_ERROR_CODES.has(errorCode)) {
              return null;
            }
            const message = error instanceof Error ? error.message : String(error);
            console.warn(`Failed to estimate diff stats for new file ${file.path}: ${message}`);
            return null;
          }
        })
      );
      ...
      return {
        ...
        ahead,
        behind,
        files: limitedStatusFiles.map((f) => ({
          path: f.path,
          index: f.index,
