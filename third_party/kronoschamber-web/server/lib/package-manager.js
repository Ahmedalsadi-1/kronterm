...
export async function getLatestVersion() {
  try {
    ...
    if (response.status === 404) {
      // Some local/enterprise builds are not published to npm.
      // Treat this as "no update metadata" instead of noisy warnings.
      return null;
    }
    ...
    if (!isNotFound) {
      console.warn('Failed to fetch latest version from npm:', message);
    }
    return null;
  }
}
