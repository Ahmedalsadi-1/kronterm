...
description: >
  ...
  collage) — the same aesthetic Krondesign uses for its own marketing
...
This is the canonical Krondesign marketing-page recipe — the example
...
worked example (Krondesign itself).
...
`pnpm --filter @krondesign/landing-page build` for a static `out/`
...
├── inputs.example.json      # Krondesign as the worked example
...
└── assets/
    ├── *.png                # 16 collage plates (Krondesign instance)
