...
Inspect and mutate a persisted KronTerm canvas graph. Every subcommand requires the canvas workspace ID and canvas block ID.

```sh
wsh canvas load <workspace-id> <block-id>
wsh canvas snapshot <workspace-id> <block-id> [--include-content]
wsh canvas save <workspace-id> <block-id> '<document-json>'
wsh canvas create-node <workspace-id> <block-id> '<node-json>'
wsh canvas update-node <workspace-id> <block-id> '<node-json>'
wsh canvas delete-node <workspace-id> <block-id> <node-id>
wsh canvas connect <workspace-id> <block-id> <from-node> <to-node> [label]
wsh canvas launch-node <workspace-id> <block-id> <node-id> <tab-id>
wsh canvas upload-asset <workspace-id> <block-id> <path> [--mime-type <type>]
```

Use `snapshot` before graph mutations. `delete-node` also removes attached edges, and `save` replaces the full document.

Examples:

```sh
wsh canvas snapshot workspace-1 block-1 --include-content
wsh canvas create-node workspace-1 block-1 '{"type":"note","title":"Investigation","content":"Start here"}'
wsh canvas connect workspace-1 block-1 node-1 node-2 "next"
wsh canvas upload-asset workspace-1 block-1 ./diagram.png
```
...
Inspect and control the active rendered workspace presentation. Unlike `blocks list`, the snapshot reports what the user
actually sees: `widgets`, `tabs`, or `canvas`, ordered widget identities, focus and visibility, screen bounds, canvas world
bounds and camera, and the tiled split tree when applicable.

```sh
wsh workspace-surface snapshot
wsh workspace-surface screenshot
wsh workspace-surface control <action> [flags]
```

Control actions:

- `set_presentation` with `--presentation widgets|tabs|canvas`
- `focus` with `--block <id>`
- `move` with `--block <id>` and either `--target-block <id> --position before|after`, a directional target, or canvas `--x/--y`
- `swap` with `--block <id> --target-block <id>`
- `resize` with `--block <id> --size 10..90` in widgets, or `--width/--height [--x/--y]` in canvas
- `magnify` with `--block <id>` in widgets
- `navigate` with `--block <id> --direction up|right|down|left`
- `fit` or `arrange` in canvas
- `add_note` with `--text`, optional `--color amber|blue|green|rose|slate`, and optional canvas geometry
- `update_object` or `delete_object` with `--object <id>`
- `connect_objects` with `--from-object <id> --to-object <id>`

Examples:

```sh
# Learn the active presentation, widget order, geometry, and supported controls
wsh workspace-surface snapshot

# Switch to canvas and arrange every live widget
wsh workspace-surface control set_presentation --presentation canvas
wsh workspace-surface control arrange

# Move and resize a widget in canvas world coordinates, including a valid zero origin
wsh workspace-surface control move --block <id> --x 0 --y 0
wsh workspace-surface control resize --block <id> --width 840 --height 540

# Build a collision-free whiteboard task chain
wsh workspace-surface control add_note --text "Investigate login" --color amber --x 0 --y 640
wsh workspace-surface snapshot
wsh workspace-surface control connect_objects --from-object <first-note-id> --to-object <second-note-id>

# Resize a tiled widget to 65% of its immediate split
wsh workspace-surface control resize --block <id> --size 65
```

Run `snapshot` again after mutations. The returned capabilities explain which controls apply to the current presentation.
