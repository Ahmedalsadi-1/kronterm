...
| app:tabbar <VersionBadge version="v0.14.4" />                  | string   | Controls the position of the tab bar: `"left"` (default) for a vertical tab bar on the left side of the window, or `"top"` for a horizontal tab bar at the top of the window                                                                                  |
| app:layoutmode <VersionBadge version="v0.15" />                | string   | Controls workspace widget presentation: `"widgets"` (default) preserves the tiled split layout, `"tabs"` shows one focused widget at a time in a browser-style tab strip, and `"canvas"` uses the spatial workspace canvas                                    |
| app:minimalui <VersionBadge version="v0.15" />                 | bool     | Set to true to enable the Minimal UI appearance mode: quiet chrome with fewer borders, flat surfaces, and content-first styling (defaults to false, also available in KronSettings under Interface)                                                           |
...
| kronoscode:endpoint <VersionBadge version="v0.15" />           | string   | optional external KronosCode URL; empty uses the managed local backend. Non-loopback endpoints must use HTTPS                                                                                                                                                 |
| kronoscode:binary <VersionBadge version="v0.15" />             | string   | optional executable override for the managed KronosCode backend                                                                                                                                                                                               |
| kronoscode:username <VersionBadge version="v0.15" />           | string   | Basic Auth username for an external KronosCode backend (default `"kronoscode"`)                                                                                                                                                                               |
| kronoscode:passwordsecretname <VersionBadge version="v0.15" /> | string   | name of the KronTerm secret containing the external KronosCode Basic Auth password                                                                                                                                                                            |
