...
id: "waveai-modes"
title: "Providers and Local Models"
---
...
KronosCode supports custom AI modes for local models, custom API endpoints, and cloud providers. The compatibility file
and setting names still use `waveai`; keep those identifiers exactly as shown in the examples below.
...
3. Choose "Models" or "Providers" from the KronSettings sidebar
...
Provider-based configuration applies defaults for known services. Set `ai:provider` to select the endpoint shape, API type,
and default secret name.
...
KronosCode's compatibility provider layer supports these API types:
...
Configure global behavior in KronSettings or `settings.json`, separately from the modes in `waveai.json`.

### Setting a Default AI Mode

After configuring a local model or custom mode, set `waveai:defaultmode` in KronTerm's settings.
...
This makes the mode the default for compatible KronosCode and AI-panel surfaces.

:::note
Custom local and BYOK modes can be used without enabling telemetry when `waveai:defaultmode` points to that custom mode.
Local inference keeps model requests on the configured local endpoint; BYOK cloud requests go directly to the selected
provider. <VersionBadge version="v0.13.1"/>
:::

### Hiding compatibility cloud modes

To show only local or custom modes, set `waveai:showcloudmodes` to `false`:
...
This hides built-in compatibility cloud modes and leaves your configured modes visible.
...
The `ai:apitokensecretname` value identifies the secret that contains your API key. Store it before starting KronTerm.
...
Instead of storing API keys directly in the configuration, use KronTerm's secret store. Supported platforms protect
secrets with the operating system's native secure storage.
...
The `openai` provider automatically looks for the `OPENAI_KEY` secret. See the [Secrets documentation](./secrets.mdx) for
...
If KronTerm cannot connect to your model server:
...
| `waveai:cloud`          | No       | Internal compatibility mode                                                                                                                                 |
| `waveai:premium`        | No       | Internal compatibility mode                                                                                                                                 |
...
:::warning
If you omit `"tools"` from `ai:capabilities`, the model cannot call compatible terminal, widget, file, or command tools.
Add it only when the endpoint and selected model support the required tool-call format.
:::
