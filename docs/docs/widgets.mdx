...
Every interactive KronTerm surface lives in a widget. A widget keeps the identity and state of its terminal, browser, file,
AI session, sandbox, or system tool while the workspace presents it as a tiled block, a focused tab, or a node on the
spatial canvas.
...
Use the widget bar or add menu in the current workspace. Tiled mode adds the widget to the split layout, tab mode adds it
to the focused pane's tab strip, and canvas mode places it near the visible camera center.
...
## Magnify or expand a widget

Magnification brings the focused widget in front of a tiled layout without destroying the saved split. On the canvas, the
expand control fills the canvas viewport and can restore the previous node size. Use the header control or

<Kbd k="Cmd:m" /> where supported.
...
For tab and canvas behavior, see [Workspace Modes and Canvas](./workspace-canvas).

## Widget types

| Widget                 | Purpose                                                                                    |
| ---------------------- | ------------------------------------------------------------------------------------------ |
| **Terminal**           | Full local or remote PTY with shell integration and AI-readable scrollback                 |
| **Browser**            | Embedded Chromium for documentation, dashboards, localhost, and agent-driven validation    |
| **Preview**            | Directories, Markdown, images, media, PDFs, CSV, and other file presentations              |
| **Code editor**        | Monaco-based text editing with local LSP services where configured                         |
| **KronosChamber**      | KronosCode chat, sessions, tools, approvals, artifacts, models, and recovery state         |
| **Kron Sandbox**       | Isolated Linux desktop with its own terminal, browser, editor, and filesystem              |
| **App stream**         | Live native or remote application content with computer-use state and cursor feedback      |
| **Launcher**           | Commands, navigation, and workspace actions                                                |
| **KronSettings**       | Models, providers, agents, MCP, skills, themes, voice, keybindings, and visual preferences |
| **System information** | CPU, memory, disk, and network monitoring                                                  |

The sections below document detailed behavior for established terminal and preview widgets.
...
Use `wsh view [path]` from a KronTerm terminal to open a preview widget with the contents of the specified path (for
example, `wsh view .` or `wsh view ~/myimage.jpg`).
