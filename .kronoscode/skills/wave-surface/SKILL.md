...
The `wave-surface` MCP server must be available. For kronoscode agents, this is set up automatically in two ways:

### ACP Integration (inside KronTerm chat)
When kronoscode agents run inside KronTerm's ACP chat, the wave-surface server is automatically registered via `session/new`. No manual config needed.

### Standalone kronoscode agents (CLI mode)
For standalone use, symlink the MCP server into kronoscode's auto-discovery path:

```bash
# The symlink is already created at:
ls -la ~/.kronoscode/mcp-servers/wave-surface

# If missing, create it:
ln -sf /Users/albsheralsadi/kronterm/mcp-wave-surface ~/.kronoscode/mcp-servers/wave-surface
```

KronosCode auto-discovers MCP servers from `~/.kronoscode/mcp-servers/`. After symlinking, the wave-surface tools are available to any kronoscode agent that loads the `wave-surface` skill.

### Verify it's working
```bash
# The MCP server build must be up-to-date:
(cd ~/.kronoscode/mcp-servers/wave-surface && npx tsc)

# Test via wsh CLI (inside a KronTerm shell):
alias wsh=/Users/albsheralsadi/kronterm/dist/bin/wsh
BLOCK_ID=$(wsh blocks list --json | jq -r '.[0].blockid')
wsh -b $BLOCK_ID widget snapshot --json
```
