...
export async function resolveGatewayWsUrl(deps: ResolveGatewayWsUrlDeps, conn: GatewayWsConnection): Promise<string> {
  ...
  if (conn.authMode === 'oauth') {
    ...
    try {
      ...
      if ('needsOauthLogin' in result && result.needsOauthLogin) {
        throw new GatewayReauthRequiredError(
          'Your remote gateway session has expired. Open Settings -> Gateway and click "Sign in" again.',
          { cause: new Error('error' in result ? result.error : undefined) }
        )
      }

      throw new Error('error' in result ? result.error || 'Could not refresh the remote gateway WebSocket ticket.' : 'Could not refresh the remote gateway WebSocket ticket.')
    } catch (error) {
