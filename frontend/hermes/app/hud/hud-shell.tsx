...
import { TitlebarIcon } from "@hermes/app/shell/titlebar-icon";
...
import { useHermesShellMode } from "@hermes/lib/kronterm-host";
import { closeHud } from "@hermes/store/hud";
import { $activeSessionAwaitingInput } from "@hermes/store/prompts";
import { $busy, $messages } from "@hermes/store/session";
...
export function HudShell() {
    const { embedded } = useHermesShellMode();
    const { t } = useI18n();
    ...
    useEffect(() => {
        ...
        const measure = () => {
            ...
            const viewportHeight = root.getBoundingClientRect().height || window.innerHeight;
            ...
            const visible = hudTranscriptHeight({
                barHeight:
                    root.querySelector<HTMLElement>('[data-slot="composer-dock"]')?.getBoundingClientRect().height ?? 0,
            ...
            setFilled(barHeight + visible >= viewportHeight - 1);
        };
    ...
    const { resizing: hudResizing, onPointerDown: onHudResizePointerDown } = useHudResizeHandle(rootRef);
    ...
    return (
        <div
            className={`relative flex ${embedded ? "h-full w-full" : "h-screen w-screen"} flex-col overflow-hidden`}
            data-hud-embedded={embedded ? "" : undefined}
            data-hud-edge={edge}
        ...
        >
          ...
          invisible. Placed and revealed entirely from styles.css. */}
            ...
            <Tip label="Dock Kronos in the workspace">
                <Button
                    aria-label="Dock Kronos in the workspace"
                    className={`${titlebarButtonClass} absolute z-20`}
                    data-hud-dock=""
                    onClick={() => void window.hermesDesktop?.hud?.dock?.()}
                    size="icon-titlebar"
                    type="button"
                    variant="ghost"
                >
                    <TitlebarIcon name="layout-sidebar-left" />
                </Button>
            </Tip>

            {embedded ? (
                <Tip label="Dismiss Kronos HUD">
                    <Button
                        aria-label="Dismiss Kronos HUD"
                        className={`${titlebarButtonClass} absolute z-20`}
                        data-hud-dismiss=""
                        onClick={() => void window.hermesDesktop?.hud?.dismiss?.()}
                        size="icon-titlebar"
                        type="button"
                        variant="ghost"
                    >
