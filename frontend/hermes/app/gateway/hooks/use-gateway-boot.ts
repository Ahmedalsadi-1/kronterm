...
import type { HermesConnection } from '@hermes/global'
import { HermesGateway } from '@hermes/hermes'
import { translateNow } from '@hermes/i18n'
import { desktopDefaultCwd } from '@hermes/lib/desktop-fs'
import { reconnectBackoffDelayMs } from '@hermes/lib/reconnect-backoff'
import {
  $desktopBoot,
...
} from '@hermes/store/gateway'
import { $gatewaySwitching, wipeSessionListsForGatewaySwitch } from '@hermes/store/gateway-switch'
import { notify, notifyError } from '@hermes/store/notifications'
import { $activeGatewayProfile, normalizeProfileKey, touchActiveGatewayBackend } from '@hermes/store/profile'
import {
  $activeSessionId,
  $connection,
  $currentCwd,
...
import { windowProfileOverride } from '@hermes/store/windows'
...
export function useGatewayBoot({
  ...
  useEffect(() => {
    ...
    // composer stayed disabled forever on "Starting KronTerm...". Once the
    ...
    const attemptReconnect = async () => {
      ...
      try {
        ...
        // "Starting KronTerm…". The probe is a no-op for a healthy or local backend.
        ...
        // as an opaque "Could not connect to KronTerm gateway". resolveGatewayWsUrl
    ...
    async function boot() {
      try {
        ...
        await Promise.all([
          ...
          // retry it), not brick boot behind the "KronTerm couldn't start"
