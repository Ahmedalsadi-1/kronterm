...
} from '@hermes/lib/generated-images'
import { parseTodos } from '@hermes/lib/todos'
import { dispatchNativeNotification } from '@hermes/store/native-notifications'
import { isDiskFullErrorMessage, notifyError } from '@hermes/store/notifications'
import { broadcastSessionsChanged } from '@hermes/store/session-sync'
import { upsertSubagent } from '@hermes/store/subagents'
import { setSessionTodos } from '@hermes/store/todos'
  ...
  const failAssistantMessage = useCallback(
    (sessionId: string, errorMessage: string, occurredAt = Date.now() / 1000) => {
      updateSessionState(sessionId, state => {
        ...
        const groupId = state.pendingBranchGroup ?? undefined
        const prev = state.messages
        const error = errorMessage.trim() || 'KronTerm reported an error'
