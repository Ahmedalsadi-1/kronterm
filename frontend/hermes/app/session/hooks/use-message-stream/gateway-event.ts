...
import { readActivePreview } from '@hermes/app/chat/right-rail/preview-reader'
import { writeAgentTerminalChunk } from '@hermes/app/right-sidebar/terminal/agent-terminal-stream'
import { readActiveTerminal } from '@hermes/app/right-sidebar/terminal/buffer'
import { closeAgentTerminalByProc } from '@hermes/app/right-sidebar/terminal/terminals'
import { burstVibeHearts } from '@hermes/components/chat/vibe-hearts'
import { translateNow } from '@hermes/i18n'
import { type GatewayEventPayload, textPart } from '@hermes/lib/chat-messages'
import { coerceGatewayText, coerceThinkingText, normalizePersonalityValue } from '@hermes/lib/chat-runtime'
import { playCompletionSound } from '@hermes/lib/completion-sound'
import {
  approvalReplaySessionId,
  resolveGatewayEventSessionId,
  UNSCOPED_STREAM_EVENT_TYPES
} from '@hermes/lib/gateway-events'
import { triggerHaptic } from '@hermes/lib/haptics'
import { modelOptionsQueryKey } from '@hermes/lib/model-options'
import { isProviderSetupErrorMessage } from '@hermes/lib/provider-setup-errors'
import { invalidateSlashCompletions } from '@hermes/lib/slash-completion-cache'
import { type AgentNoticePayload, clearAgentNotice, nativeNoticeInput, showAgentNotice } from '@hermes/store/agent-notices'
import { reconcileApprovalModeForProfile } from '@hermes/store/approval-mode'
import { billingCtaLabel, clearBillingBlock, runBillingRecovery, setBillingBlock } from '@hermes/store/billing-block'
import { clearClarifyRequest, normalizeChoices, setClarifyRequest, warnDroppedChoices } from '@hermes/store/clarify'
import { setSessionCompacting } from '@hermes/store/compaction'
import { refreshBackgroundProcesses } from '@hermes/store/composer-status'
import { $gateway, activeGatewayConnectionId } from '@hermes/store/gateway'
import { applyGoalStatusText } from '@hermes/store/goals'
import {
  notifyCronChanged,
...
} from '@hermes/store/live-sync'
import { setMcpSetupRequest } from '@hermes/store/mcp-setup'
import { dispatchNativeNotification } from '@hermes/store/native-notifications'
import { isDiskFullErrorMessage, notify, notifyError } from '@hermes/store/notifications'
import { requestDesktopOnboarding, requestDesktopOnboardingForCredentialWarning } from '@hermes/store/onboarding'
import { revealDesktopPane } from '@hermes/store/pane-focus'
import { flashPetActivity, markPetUnread, setPetActivity } from '@hermes/store/pet'
import { $activeGatewayProfile, normalizeProfileKey } from '@hermes/store/profile'
import { followActiveSessionCwd } from '@hermes/store/projects'
import {
  clearAllPrompts,
  ...
  setSecretRequest,
  setSudoRequest
} from '@hermes/store/prompts'
import { providerWaitText, setSessionProviderWait } from '@hermes/store/provider-wait'
import { recordAgentReaction } from '@hermes/store/reactions-local'
import {
  $currentCwd,
  $currentModel,
  $currentProvider,
...
import { dropSessionState } from '@hermes/store/session-states'
import { pruneDelegateFallbackSubagents, pruneFinishedSessionSubagents, upsertSubagent } from '@hermes/store/subagents'
import { reportMcpToolResult } from '@hermes/store/suggestion-providers/repair'
import { invalidateSkillSuggestionIndex } from '@hermes/store/suggestion-providers/skill'
import { requestScrollToBottom } from '@hermes/store/thread-scroll'
import { clearActiveSessionTodos } from '@hermes/store/todos'
import { recordToolDiff } from '@hermes/store/tool-diffs'
import { setSessionDraftingTool } from '@hermes/store/tool-drafting'
import { reportInstallMethodWarning } from '@hermes/store/updates'
import { notifyWorkspaceChanged, toolChangedPath, toolMayMutateFiles } from '@hermes/store/workspace-events'
// Leaf import (not the `@/themes` barrel) to avoid pulling the ThemeProvider
// module graph into the gateway event hot path.
import { ingestBackendSkin } from '@hermes/themes/backend-sync'
import type { RpcEvent } from '@hermes/types/hermes'
 ...
 * billing-specific toast — never the generic "KronTerm error" — with a smart CTA
      ...
      if (event.type === 'gateway.ready') {
        ...
        const failure =
          payload?.status === 'error'
            ? {
                error: coerceGatewayText(payload.error).trim() || finalText || 'KronTerm reported an error',
        ...
        const errorMessage = payload?.message || 'KronTerm reported an error'
