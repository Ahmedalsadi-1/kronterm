...
export function useDowngradeFlow({ onScheduled }: { onScheduled: () => void }) {
  ...
  const runPreview = async (target: DowngradeTarget, runId: number) => {
    ...
    setActive({
      phase: result.ok ? { kind: 'ready', preview: result.data } : { kind: 'previewFailed', refusal: 'refusal' in result ? result.refusal : undefined },
      target
    })
  }
  ...
  const confirm = async () => {
    ...
    if (!result.ok) {
      ...
      setActive({ phase: { kind: 'scheduleFailed', preview, refusal: 'refusal' in result ? result.refusal : undefined }, target })
...
export function useResumeFlow() {
  ...
  const resume = async () => {
    ...
    if (!result.ok) {
      ...
      setBusy(false)
      setRefusal('refusal' in result ? result.refusal : undefined)
