...
export async function pollChargeSettlement(
  ...
  const settlement = await driveChargeSettlement({
    fetchStatus: async () => {
      ...
      observed.refusal = 'refusal' in result ? result.refusal : undefined
      observed.status = statusFromRefusal('refusal' in result ? result.refusal : undefined)
...
export function useChargeFlow() {
  ...
  const start = useCallback(
    async (amountUsd: string) => {
      ...
      if (!chargeResult.ok) {
        const resolved = resolveRefusal('refusal' in chargeResult ? chargeResult.refusal : undefined)
        ...
        retryIntentRef.current = shouldReuseIdempotencyKey('refusal' in chargeResult ? chargeResult.refusal : undefined)
