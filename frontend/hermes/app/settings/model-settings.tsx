...
import { Select, SelectContent, SelectItem, SelectTrigger, SelectValue } from "@hermes/components/ui/select";
import { Skeleton } from "@hermes/components/ui/skeleton";
...
import { AlertTriangle, Cpu, Loader2 } from "@hermes/lib/icons";
import { DEFAULT_REASONING_EFFORT, REASONING_EFFORT_VALUES } from "@hermes/lib/reasoning-effort";
import { cn } from "@hermes/lib/utils";
import { setMainModelAssignment } from "@hermes/store/cron-model-impact";
import { notifyError } from "@hermes/store/notifications";
import { startManualLocalEndpoint, startManualOnboarding, startManualProviderOAuth } from "@hermes/store/onboarding";
    ...
    return (
        <div className="flex flex-wrap items-center gap-2 rounded-md border border-amber-500/40 bg-amber-500/10 px-3 py-2 text-xs text-amber-200">
            ...
            <span className="grow">
                ...
                <span className="font-mono">{allSameProvider ? provider : "other providers"}</span>, not your main
                model.
            </span>
...
export function ModelSettings({ onMainModelChanged }: ModelSettingsProps) {
    ...
    const currentMoaPreset = useMemo(() => {
        ...
        return (
            moa.presets[selectedMoaPreset] || moa.presets[moa.default_preset] || Object.values(moa.presets)[0] || null
        );
                ...
                {needsSetup && !setupIsApiKey && selectedProviderRow && (
                    <p className="mt-2 text-xs text-muted-foreground">
                        {selectedProviderRow?.auth_type === "api_key"
                            ...
                            : `${selectedProviderRow?.name} signs in through your browser — Kronos runs the flow for you.`}
                    </p>
                )}
                                        ...
                                        isEditing && (
                                            <div className="mt-2 flex flex-wrap items-center gap-2 pt-1">
                                                ...
                                                >
                                                    ...
                                                    <SelectContent>
                                                        {withActive(auxDraftProviderModels, auxDraft.model).map(
                                                            (model) => (
                                                                <SelectItem key={model} value={model}>
                                                                    {model}
                                                                </SelectItem>
            ...
            {moa && currentMoaPreset && (
                <section>
                    ...
                    <p className="mb-2 text-xs text-muted-foreground">
                        Configure named presets that appear as models under the Mixture of Agents provider. The
                        aggregator is the acting model.
                    </p>
                        ...
                        <Button
                            ...
                            onClick={() => {
                                ...
                                const next: MoaConfigResponse = {
                                    ...
                                    presets,
                                    default_preset:
                                        moa.default_preset === selectedMoaPreset ? fallback : moa.default_preset,
                                    active_preset: moa.active_preset === selectedMoaPreset ? "" : moa.active_preset,
                                };

                                setSelectedMoaPreset(
                                    Object.keys(moa.presets).find((name) => name !== selectedMoaPreset) || ""
                                );
                                void saveMoa(next);
                            }}
                            size="sm"
                                ...
                                below={
                                    <div className="mt-2 flex flex-wrap items-center gap-2 pt-1">
                                        ...
                                        >
                                            ...
                                            <SelectContent>
                                                {withActive(modelsForProvider(slot.provider), slot.model).map(
                                                    (model) => (
                                                        <SelectItem key={model} value={model}>
                                                            {model}
                                                        </SelectItem>
