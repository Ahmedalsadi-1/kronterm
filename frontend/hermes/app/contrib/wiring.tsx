...
import { graftRefreshedTailOntoBackfill } from "@hermes/app/chat/transcript-backfill";
import { formatRefValue } from "@hermes/components/assistant-ui/directive-text";
import { BootFailureOverlay } from "@hermes/components/boot-failure-overlay";
import { DesktopInstallOverlay } from "@hermes/components/desktop-install-overlay";
import { FindBar } from "@hermes/components/find-bar";
import { GatewayConnectingOverlay } from "@hermes/components/gateway-connecting-overlay";
import { NotificationStack } from "@hermes/components/notifications";
import { DesktopOnboardingOverlay } from "@hermes/components/onboarding";
import { $newSessionTabAction, registerPaneCloser } from "@hermes/components/pane-shell/tree/store";
import { FloatingPet } from "@hermes/components/pet/floating-pet";
import { RemoteDisplayBanner } from "@hermes/components/remote-display-banner";
...
import {
    ...
    toChatMessages,
} from "@hermes/lib/chat-messages";
import { isKronTermWidgetHost, useHermesShellMode } from "@hermes/lib/kronterm-host";
...
import { playWakeSound } from "@hermes/lib/wake-sound";
import { $billingSettingsRequest } from "@hermes/store/billing-block";
import { $desktopBoot } from "@hermes/store/boot";
import { requestVoiceConversationStart } from "@hermes/store/composer";
import { $cronReviewRequest, setCronFocusJobId } from "@hermes/store/cron";
import { $pinnedSessionIds, pinSession, restoreWorktree, unpinSession } from "@hermes/store/layout";
...
import { $startWorkSessionRequest, followActiveSessionCwd } from "@hermes/store/projects";
import {
    $activeSessionId,
    $connection,
    $currentCwd,
...
import { clearSessionTodos, setSessionTodos, todosForHydration } from "@hermes/store/todos";
import { armWakeWord, stopClientCapture } from "@hermes/store/wake-word";
import { isAuxiliaryWindow, isHudWindow } from "@hermes/store/windows";
import { useSkinCommand } from "@hermes/themes/use-skin-command";
    ...
    return (
        <ContribWiringContext.Provider value={api}>
          ...
          Never in the HUD: that window is the chat bar and nothing else. */}
            {!hudMode && !isHudWindow() && <FloatingPet />}
          ...
          The HUD has no terminal pane, so it has nothing to chase. */}
            {!hudMode && !isHudWindow() && !isKronTermWidgetHost() && (
