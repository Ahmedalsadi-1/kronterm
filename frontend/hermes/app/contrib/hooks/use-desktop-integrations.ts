...
import { storedSessionIdForNotification } from "@hermes/lib/session-ids";
import { requestMcpInstallFromDeepLink } from "@hermes/store/mcp-deeplink-install";
import { startMcpHealthChecker, stopMcpHealthChecker } from "@hermes/store/mcp-health";
import { respondToApprovalAction } from "@hermes/store/native-notifications";
...
import { onSessionsChanged } from "@hermes/store/session-sync";
import { openUpdatesWindow, startUpdatePoller, stopUpdatePoller } from "@hermes/store/updates";
import { isHudWindow, isSecondaryWindow } from "@hermes/store/windows";
...
export function useDesktopIntegrations({
    ...
    useEffect(() => {
        if (!profileReady || hudMode || isHudWindow()) {
            return;
        }
    ...
    }, [activeProfile, hudMode, locationPathname, navigate, profileReady, routedSessionId, sessions]);
