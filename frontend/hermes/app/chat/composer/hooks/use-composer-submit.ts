...
import { SLASH_COMMAND_RE } from "@hermes/lib/chat-runtime";
import { triggerHaptic } from "@hermes/lib/haptics";
import { hasClarifyRequest, skipClarifyRequest } from "@hermes/store/clarify";
import { clearSessionDraft, type ComposerAttachment } from "@hermes/store/composer";
import { resetBrowseState } from "@hermes/store/composer-input-history";
import { enqueueQueuedPrompt, type QueuedPromptEntry } from "@hermes/store/composer-queue";
import { hasMcpSetupRequest, skipMcpSetupRequest } from "@hermes/store/mcp-setup";
import { hasBlockingPromptRequest } from "@hermes/store/prompts";
...
export const HermesComposerSubmittedEvent = "hermes:composer-submitted";

export type HermesComposerSubmitted = {
    attachments: ComposerAttachment[];
    text: string;
};

function publishComposerSubmitted(text: string, attachments: ComposerAttachment[]): void {
    window.dispatchEvent(
        new CustomEvent<HermesComposerSubmitted>(HermesComposerSubmittedEvent, {
            detail: { attachments, text },
        })
    );
}
    ...
    const dispatchSubmit = (text: string, attachments?: ComposerAttachment[]) => {
        ...
        void Promise.resolve(
            ...
            .then((accepted) => {
                ...
                publishComposerSubmitted(text, submittedAttachments);
