...
import { refChipLabel } from "@hermes/components/assistant-ui/directive-text";
import type { HermesKronTermSurface } from "@hermes/global";
import type { HermesGateway } from "@hermes/hermes";
import { cachedPathCompletion, hasCachedPathCompletion } from "@hermes/lib/slash-completion-cache";
import { normalize } from "@hermes/lib/text";

import type { CompletionEntry, CompletionPayload } from "./use-live-completion-adapter";
import { useLiveCompletionAdapter } from "./use-live-completion-adapter";

const KIND_RE = /^@(file|folder|url|image|tool|git|widget):(.*)$/;
const REF_STARTERS = new Set(["file", "folder", "url", "image", "tool", "git", "widget"]);
...
const STARTER_META: Record<string, string> = {
    ...
    widget: "Mention a live KronTerm widget",
};
...
export function liveSurfaceEntries(surfaces: HermesKronTermSurface[], query: string): CompletionEntry[] {
    const filter = normalize(query);
    return surfaces.flatMap((surface) => {
        const widget: CompletionEntry = {
            text: `@widget:${surface.id}`,
            display: surface.title,
            meta: `${surface.view}${surface.active ? " · agent active" : ""}`,
        };
        const browserTabs: CompletionEntry[] = (surface.browserTabs ?? []).map((tab) => ({
            text: `@url:${tab.url}`,
            display: tab.title || tab.url,
            meta: `Browser tab · ${surface.title}${tab.active ? " · active" : ""}`,
        }));
        return [widget, ...browserTabs].filter(
            (entry) => !filter || normalize(`${entry.display} ${entry.meta} ${entry.text}`).includes(filter)
        );
    });
}

function uniqueEntries(entries: CompletionEntry[]): CompletionEntry[] {
    const seen = new Set<string>();
    return entries.filter((entry) => {
        const key = `${entry.text}\u0000${entry.display ?? ""}\u0000${entry.meta ?? ""}`;
        if (seen.has(key)) {
            return false;
        }
        seen.add(key);
...
export function useAtCompletions(options: {
    ...
    const enabled = Boolean(gateway || window.hermesDesktop?.krontermSurfaces);
    ...
    const fetcher = useCallback(
        async (query: string): Promise<CompletionPayload> => {
            ...
            const surfaces = await window.hermesDesktop?.krontermSurfaces?.list().catch(() => []);

            if (query.startsWith("widget:")) {
                const filter = normalize(query.slice("widget:".length));
                const items = (surfaces ?? [])
                    .filter((surface) => !filter || normalize(`${surface.title} ${surface.view}`).includes(filter))
                    .map((surface) => ({
                        text: `@widget:${surface.id}`,
                        display: surface.title,
                        meta: `${surface.view}${surface.active ? " · agent active" : ""}`,
            ...
            const surfaceItems = query.includes(":") ? [] : liveSurfaceEntries(surfaces ?? [], query);

            if (!gateway) {
                return { items: uniqueEntries([...surfaceItems, ...starters]), query };
            }
            ...
            try {
                ...
                return { items: uniqueEntries([...surfaceItems, ...(items.length > 0 ? items : starters)]), query };
            } catch {
                return { items: uniqueEntries([...surfaceItems, ...starters]), query };
            }
        },
    ...
    const isCached = useCallback(
        (query: string) =>
            query.includes(":") && !query.startsWith("widget:") && hasCachedPathCompletion(cacheKey(query)),
        [cacheKey]
    );
