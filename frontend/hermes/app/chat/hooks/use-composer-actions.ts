...
import { attachmentId, contextPath, pathLabel } from "@hermes/lib/chat-runtime";
import { readDesktopFileDataUrlLocalFirst, selectDesktopPaths } from "@hermes/lib/desktop-fs";
import { desktopGit } from "@hermes/lib/desktop-git";
import { downscaleDataUrlForPreview } from "@hermes/lib/image-resize";
...
import {
    ...
    publishComposerAttachmentUserRemoved,
...
export function useComposerActions({
    ...
    const attachDroppedItems = useCallback(
        async (candidates: DroppedFile[]) => {
            ...
            for (const candidate of candidates) {
                ...
                const isImage =
                    file.type.startsWith("image/") || isImagePath(file.name) || (filePath && isImagePath(filePath));
    ...
    const removeAttachment = useCallback(
        async (id: string) => {
            ...
            if (removed) {
                publishComposerAttachmentUserRemoved(removed);
            }
