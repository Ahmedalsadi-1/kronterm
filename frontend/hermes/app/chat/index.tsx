...
import type { SubmitTextOptions } from '@hermes/app/session/hooks/use-prompt-actions/utils'
import { Thread } from '@hermes/components/assistant-ui/thread'
import { TranscriptWindowProvider } from '@hermes/components/assistant-ui/thread/transcript-window'
import { Backdrop } from '@hermes/components/Backdrop'
import { COMPOSER_HEART_CONFIG, HeartField } from '@hermes/components/chat/vibe-hearts'
import { usePaneVisible } from '@hermes/components/pane-shell/pane-visibility'
import { $sessionTileDragging, $sessionTileEdgeHover } from '@hermes/components/pane-shell/tree/store'
import { PromptOverlays } from '@hermes/components/prompt-overlays'
import { Button } from '@hermes/components/ui/button'
import { ErrorState } from '@hermes/components/ui/error-state'
import { TitleMenuTrigger } from '@hermes/components/ui/title-menu-trigger'
import { type HermesGateway } from '@hermes/hermes'
import { useI18n } from '@hermes/i18n'
import type { ChatMessage } from '@hermes/lib/chat-messages'
import { NEW_SESSION_TITLE, quickModelOptions, sessionTitle } from '@hermes/lib/chat-runtime'
import { useIncrementalExternalStoreRuntime } from '@hermes/lib/incremental-external-store-runtime'
import { modelOptionsQueryKey, requestModelOptions } from '@hermes/lib/model-options'
import { cn } from '@hermes/lib/utils'
import { migrateSessionDraft } from '@hermes/store/composer'
import { migrateQueuedPrompts, parkQueuedPrompts } from '@hermes/store/composer-queue'
import { $pinnedSessionIds } from '@hermes/store/layout'
import { $petActive } from '@hermes/store/pet'
import { $petOverlayActive } from '@hermes/store/pet-overlay'
import { $activeGatewayProfile, $gatewaySwapTarget, $profiles } from '@hermes/store/profile'
import {
  $contextSuggestions,
  $freshDraftReady,
  $gatewayState,
...
import { sessionTileDelegate } from '@hermes/store/session-states'
import { $transcriptTailBySessionId } from '@hermes/store/transcript-tail'
import { isAuxiliaryWindow, isWatchWindow } from '@hermes/store/windows'
import type { ModelOptionsResponse } from '@hermes/types/hermes'
...
function ChatHeader({
  ...
  return (
    <header className={cn(titlebarHeaderBaseClass, 'overflow-visible', isRoutedSessionView && titlebarHeaderShadowClass)}>
      <div
        className={cn(titlebarHeaderTitleClass, 'overflow-visible', showProfileTag && 'flex items-center')}
        style={{
          maxWidth:
