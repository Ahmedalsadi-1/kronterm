...
import { PrTag } from '@hermes/app/chat/pr-tag'
import { ProfileTag } from '@hermes/app/chat/profile-tag'
import { startSessionDrag } from '@hermes/app/chat/session-drag'
import { PlatformAvatar } from '@hermes/app/messaging/platform-icon'
import { openSession } from '@hermes/app/open-session'
import { formatMessageTimestamp } from '@hermes/components/assistant-ui/thread/timestamp'
import { Button } from '@hermes/components/ui/button'
import { Codicon } from '@hermes/components/ui/codicon'
import { OverflowTip, Tip } from '@hermes/components/ui/tooltip'
import type { SessionInfo } from '@hermes/hermes'
import { type Translations, useI18n } from '@hermes/i18n'
import { sessionTitle } from '@hermes/lib/chat-runtime'
import { pathLeaf } from '@hermes/lib/display-path'
import { compactNumber } from '@hermes/lib/format'
import { triggerHaptic } from '@hermes/lib/haptics'
import { middleClickHandlers } from '@hermes/lib/middle-click'
import { displayModelName } from '@hermes/lib/model-status-label'
import { sessionProjectLabel } from '@hermes/lib/session-project-label'
import { handoffOriginSource, sessionSourceLabel } from '@hermes/lib/session-source'
import { coarseElapsed } from '@hermes/lib/time'
import { useStoreSelector } from '@hermes/lib/use-session-slice'
import { cn } from '@hermes/lib/utils'
import { $sidebarRowMeta } from '@hermes/store/layout'
import { normalizeProfileKey } from '@hermes/store/profile'
import { $projects } from '@hermes/store/projects'
import { $pullRequestsByBranch, sessionPrKey } from '@hermes/store/pull-requests'
import { $sessionDotStateById, hasLiveTurn, showsRunningArc } from '@hermes/store/session-dot-state'
import { $sessionListDensity } from '@hermes/store/session-list-density'
import { sessionCostUsd } from '@hermes/store/sidebar-archive'
import { $todoProgressBySession } from '@hermes/store/todos'
