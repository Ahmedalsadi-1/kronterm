...
import { requestComposerFocus, requestComposerInsertRefs } from '@hermes/app/chat/composer/focus'
import { droppedFileInlineRef } from '@hermes/app/chat/composer/inline-refs'
import { HERMES_PATHS_MIME } from '@hermes/app/chat/hooks/use-composer-actions'
import { isAddSelectionShortcut } from '@hermes/app/right-sidebar/terminal/selection'
import { RichCodeBlock } from '@hermes/components/assistant-ui/embeds'
import { CodeEditor } from '@hermes/components/chat/code-editor'
import { FileDiffPanel } from '@hermes/components/chat/diff-lines'
import { chunkTextLines, useFixedRowWindow } from '@hermes/components/chat/fixed-row-window'
import { LazyShiki as ShikiHighlighter } from '@hermes/components/chat/shiki-highlighter'
import { PageLoader } from '@hermes/components/page-loader'
import { Tip } from '@hermes/components/ui/tooltip'
import { translateNow, useI18n } from '@hermes/i18n'
import {
  desktopFileDiff,
  desktopFsCacheKey,
  desktopGitRoot,
...
} from '@hermes/lib/desktop-fs'
import { Check, Pencil, X } from '@hermes/lib/icons'
import { shikiLanguageForFilename } from '@hermes/lib/markdown-code'
import { cn } from '@hermes/lib/utils'
import type { PreviewTarget } from '@hermes/store/preview'
import { setPreviewDirty } from '@hermes/store/preview-edit'
import { $connection, $currentCwd } from '@hermes/store/session'
import { notifyWorkspaceChanged } from '@hermes/store/workspace-events'
...
function MarkdownPreview({ text }: { text: string }) {
  return (
    <div className="preview-markdown mx-auto max-w-3xl px-4 py-3 text-sm text-foreground" data-selectable-text="true">
      <Streamdown components={MARKDOWN_COMPONENTS as Components} controls={false} mode="static" parseIncompleteMarkdown={false}>
        {text}
      </Streamdown>
    </div>
  )
