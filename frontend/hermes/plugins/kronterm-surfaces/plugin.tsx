...
export function AgentSurfaceMicroWindow() {
  ...
  const popOut = async () => {
    setError('')
    try {
      await host.krontermSurfaces.focus(surface.id)
      setExpanded(false)
      await refetch()
    } catch (failure) {
  ...
  return (
    <section
      ...
      className="mb-1 overflow-hidden rounded-xl border border-(--ui-accent)/30 bg-(--ui-bg-secondary)/92 shadow-[0_14px_40px_rgba(0,0,0,0.18)] backdrop-blur-xl"
    ...
    >
      <div className="flex min-h-11 items-center gap-2 p-1.5">
        ...
        <button
          ...
          type="button"
        >
          <span className="flex items-center gap-1.5 text-[0.625rem] font-semibold uppercase tracking-[0.12em] text-(--ui-accent)">
            <span className="size-1.5 animate-pulse rounded-full bg-(--ui-accent)" aria-hidden="true" />
            Agent surface
          </span>
      ...
      {expanded ? (
        <div className="border-t border-(--ui-border) p-1.5 pt-0">
          <div className="relative aspect-video max-h-64 w-full overflow-hidden rounded-lg border border-(--ui-border) bg-black/30">
            {previewImageUrl ? (
              <img
              ...
              <div className="grid h-full place-items-center text-xs text-(--ui-text-tertiary)" role="status">
      ...
      {error ? <p className="border-t border-red-500/20 px-2.5 py-1.5 text-[0.625rem] text-red-400">{error}</p> : null}
    </section>
  )
}
...
function SurfacesPane() {
  ...
  const focus = async (blockId: string) => {
    ...
    try {
      await host.krontermSurfaces.focus(blockId)
      await refetch()
    } catch (focusFailure) {
      ...
      <div className="min-h-0 flex-1 overflow-y-auto p-2">
        {isLoading ? (
          ...
          <div className="rounded-md border border-(--ui-border) p-3 text-xs">
            ...
            <button
              className="mt-2 cursor-pointer text-(--ui-accent) hover:underline focus-visible:outline focus-visible:outline-2"
            ...
            {surfaces.map((surface) => (
              <button
                ...
                className={cn(
                  ...
                  'focus-visible:outline focus-visible:outline-2 focus-visible:outline-offset-1 focus-visible:outline-(--ui-accent)',
                  surface.focused
                    ? 'border-(--ui-accent)/45 bg-(--ui-accent)/10'
              ...
              >
                ...
                {surface.focused ? (
                  <span className="text-[0.625rem] font-medium text-(--ui-accent)">active</span>
                ) : null}
              </button>
            ))}
          </div>
