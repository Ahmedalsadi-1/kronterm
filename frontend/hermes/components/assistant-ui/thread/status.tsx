...
import { toolPresentVerb } from '@hermes/components/assistant-ui/tool/run-summary'
...
import { SCAFFOLD_LABEL_CLASS } from '@hermes/components/chat/scaffold-row'
import { Codicon } from '@hermes/components/ui/codicon'
import { Loader } from '@hermes/components/ui/loader'
import { StatusPulse } from '@hermes/components/ui/status-pulse'
import { useI18n } from '@hermes/i18n'
import { cn } from '@hermes/lib/utils'
import { $backgroundResume } from '@hermes/store/background-delegation'
import { sessionCompacting } from '@hermes/store/compaction'
import { sessionAwaitingInput } from '@hermes/store/prompts'
import { sessionProviderWait } from '@hermes/store/provider-wait'
import { $turnStartedAt } from '@hermes/store/session'
import { type DraftingTool, sessionDraftingTool } from '@hermes/store/tool-drafting'
...
export const StreamStallIndicator: FC = () => {
  ...
  return (
    <StatusRow data-slot="aui_stream-stall" label={hint || 'KronTerm is thinking'}>
      <StatusPulse
        aria-hidden="true"
        className="dither inline-block size-3 rounded-[2px] text-midground/80"
        kind="opacity"
