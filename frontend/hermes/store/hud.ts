...
import { requestComposerDraftSync } from "@hermes/store/composer";
import { $activeGatewayProfile, normalizeProfileKey } from "@hermes/store/profile";
import { $sessions, rememberedSessionProfile } from "@hermes/store/session";
...
export function setEmbeddedHudMode(enabled: boolean): void {
    $hudMode.set(enabled || isHudWindow());
}
