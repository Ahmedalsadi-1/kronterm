...
import { evaluateRuntimeReadiness, type RuntimeReadinessResult } from "@hermes/lib/runtime-readiness";
...
import type { ModelOptionProvider, OAuthProvider, OAuthStartResponse } from "@hermes/types/hermes";
...
function notifyReady(provider: string) {
    notify({ kind: "success", title: "Kronos is ready", message: `${provider} connected.` });
}
...
export async function refreshOnboarding(ctx: OnboardingContext) {
    ...
    if (shouldPreserveConfiguredOnFallback(runtime, state)) {
        ...
        notify({
            ...
            message:
                "Kronos could not verify its backend on startup. Some features may be unavailable until the gateway is reachable.",
        });
