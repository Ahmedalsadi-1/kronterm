...
export const zh: Translations = {
  ...
  boot: {
    ...
    steps: {
      ...
      startingHermesDesktop: '正在启动 KronTerm 桌面版…'
    },
    errors: {
  ...
  settings: {
    ...
    plugins: {
      title: '桌面插件',
      blurb:
        '加载到此应用中的界面扩展——随构建捆绑，或放入 desktop-plugins 文件夹（包括 KronTerm 编写的插件）。禁用会即时卸载插件并在重启后保持。',
      count: n => `已安装 ${n} 个`,
      openFolder: '打开插件文件夹',
      rescan: '重新扫描',
      reveal: '在文件管理器中显示',
      ...
      agent: {
        title: '智能体插件',
        blurb:
          '运行在 KronTerm 后端——工具、技能、MCP 服务器、钩子和斜杠命令。「便携」插件是 Agent Plugins 标准包（技能 + MCP 组合，也可在其他智能体中使用）。开关在新会话中生效。',
        empty: '尚未安装智能体插件。',
    ...
    appearance: {
      ...
      pet: {
        title: '宠物',
        intro:
          '领养一只悬浮在应用上的 petdex 动画宠物，它会根据 KronTerm 的状态做出反应——工具执行时奔跑、成功时欢呼、出错时沮丧。',
        restartHint: '宠物功能需要重启——当前运行的应用在此功能加入前启动。请退出并重新打开 KronTerm，然后回到此处。',
        scaleTitle: '大小',
    ...
    gateway: {
      ...
      envOverride: '环境变量覆盖',
      intro:
        'KronTerm Desktop 默认会启动自己的本地网关。当你希望此应用控制另一台机器上或可信代理后的现有 KronTerm 后端时，可以使用远程网关。下面可按 profile 指定各自的远程主机。',
      appliesTo: '应用于',
  ...
  messaging: {
    ...
    platformIntro: {
      ...
      weixin:
        '运行 `hermes gateway setup`，选择 Weixin，然后使用个人微信账号扫描并确认二维码。KronTerm 会通过腾讯 iLink Bot API 连接并保存凭据。',
  ...
  onboarding: {
    ...
    autoBrowser: provider => `已在浏览器中打开 ${provider}。请在那里授权 KronTerm，连接会自动完成，无需复制或粘贴。`,
