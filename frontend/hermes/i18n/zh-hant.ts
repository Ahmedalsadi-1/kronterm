...
export const zhHant = defineLocale({
    ...
    boot: {
        ...
        steps: {
            ...
            startingHermesDesktop: "正在啟動 Kronos…",
        },
        errors: {
    ...
    settings: {
        ...
        appearance: {
            ...
            pet: {
                title: "寵物",
                intro: "領養一隻懸浮在應用上的 petdex 動畫寵物，它會根據 KronTerm 的狀態做出反應——工具執行時奔跑、成功時歡呼、出錯時沮喪。",
                restartHint:
                    "寵物功能需要重新啟動——目前執行的應用在此功能加入前啟動。請結束並重新開啟 KronTerm，然後回到此處。",
                scaleTitle: "大小",
        ...
        gateway: {
            ...
            title: "閘道連線",
            envOverride: "環境變數覆寫",
            intro: "KronTerm Desktop 預設會啟動自己的本機閘道。如果您希望此應用程式控制另一台機器或可信代理後面已執行的 KronTerm 後端，請使用遠端閘道。在下方按設定檔指定各自的遠端主機。",
            appliesTo: "套用至",
            allProfiles: "全部設定檔",
            defaultConnection: "預設連線適用於所有沒有自訂覆寫的設定檔。",
            profileConnection: (profile) =>
                `僅當「${profile}」為作用中設定檔時使用此連線。選擇「使用預設閘道」可移除其覆寫。`,
        ...
        mcp: {
            ...
            deepLinkDescription:
                "一個連結要求將此 MCP 伺服器加入 KronTerm。請檢查下方的完整設定——它來自該連結，而非 KronTerm。",
    ...
    onboarding: {
        ...
        autoBrowser: (provider) =>
            `已在瀏覽器中開啟 ${provider}。請在那裡授權 KronTerm，連線會自動完成，無需複製或貼上。`,
