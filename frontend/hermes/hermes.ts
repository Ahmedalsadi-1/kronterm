...
import { recordTranscriptTail } from '@hermes/store/transcript-tail'
import type {
  ActionResponse,
...
export class HermesGateway extends JsonRpcGatewayClient {
  constructor() {
    super({
      closedErrorMessage: 'KronTerm gateway connection closed',
      connectErrorMessage: 'Could not connect to KronTerm gateway',
      createRequestId: nextId => nextId,
      notConnectedErrorMessage: 'KronTerm gateway is not connected',
...
export async function pluginRest<T>(pluginId: string, path: string, opts: PluginRestOptions = {}): Promise<T> {
  if (!window.hermesDesktop?.api) {
    throw new Error('KronTerm desktop bridge unavailable')
  }
