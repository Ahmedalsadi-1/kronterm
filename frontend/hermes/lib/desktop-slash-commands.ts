...
const DESKTOP_COMMAND_SPECS: readonly DesktopCommandSpec[] = [
    ...
    { name: "/profile", description: "Switch the active Kronos profile", surface: action("profile") },
    {
        name: "/skin",
    ...
    { name: "/version", description: "Show KronTerm version", surface: exec() },
    ...
    const categories = catalog.categories
        ?.map((section) => ({
            ...
            pairs: section.pairs
                ...
                .map(
                    ([command, description]) =>
                        [command, desktopSlashDescription(command, description)] as [string, string]
                ),
        }))
