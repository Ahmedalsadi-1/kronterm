...
import { useI18n } from "@/app/components/hermes-ui/i18n";
import { Download } from "@/app/components/hermes-ui/icons";
import { notify, notifyError } from "@/app/components/hermes-ui/notifications";
import { Dialog, DialogContent } from "@/app/components/hermes-ui/ui/dialog";
...
export function ZoomableImage({ className, containerClassName, src, alt, slot, ...props }: ZoomableImageProps) {
    ...
    async function handleDownload() {
        ...
        try {
            const hermesDesktop = (
                window as Window & { hermesDesktop?: { saveImageFromUrl?: (src: string) => Promise<boolean> } }
            ).hermesDesktop;
            if (hermesDesktop?.saveImageFromUrl) {
                const saved = await hermesDesktop.saveImageFromUrl(src);
