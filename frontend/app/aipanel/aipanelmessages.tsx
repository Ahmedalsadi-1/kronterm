...
import { reportDesktopPetActivity } from "./desktop-pet-activity";
...
export const AIPanelMessages = memo(
    ({ messages, status, onContextMenu, showModeDropdown = true }: AIPanelMessagesProps) => {
        ...
        useEffect(() => {
            ...
            if (lastMessage?.role !== "assistant") {
                reportDesktopPetActivity({ kind: "thinking" });
                return;
            }
            const toolPart = [...(lastMessage.parts ?? [])].reverse().find((part) => part.type === "data-tooluse");
            if (toolPart?.type === "data-tooluse") {
                reportDesktopPetActivity({ kind: "tool", detail: toolPart.data.toolname }, toolPart.data.blockid);
                return;
            }
            const reasoningPart = [...(lastMessage.parts ?? [])].reverse().find((part) => part.type === "reasoning");
            reportDesktopPetActivity({
                kind: "thinking",
                thought: reasoningPart?.type === "reasoning" ? reasoningPart.text : undefined,
            });
        }, [messages, status]);
