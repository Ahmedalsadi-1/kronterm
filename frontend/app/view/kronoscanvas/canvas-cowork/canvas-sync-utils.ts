import type { CanvasFSEvent } from "./protocol";
...
export function ensureUniquePath(desiredPath: string, existingPaths: Set<string>, reservedPath?: string): string {
...
export function detectMovesEnhanced(
    ...
    const metaMatches = (delMeta: MoveMeta, create: CanvasFSEvent) => {
        ...
        return delMeta.size === create.size && Math.round(delMeta.mtimeMs) === Math.round(create.mtimeMs);
    };
