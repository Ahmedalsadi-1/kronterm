...
export class NoNestFrameShapeUtil extends FrameShapeUtil {
    ...
    override onDragShapesIn(shape: TLFrameShape, draggingShapes: TLShape[], info: TLDragShapesInInfo): void {
    ...
    override indicator(shape: TLFrameShape) {
        return <rect width={toDomPrecision(shape.props.w)} height={toDomPrecision(shape.props.h)} rx={8} ry={8} />;
    }
...
function FrameComponent({ shape }: { shape: TLFrameShape }) {
    ...
    const isSelected = useValue("isSelected", () => editor.getSelectedShapeIds().includes(id), [editor, id]);

    const isEditing = useValue("isEditing", () => editor.getEditingShapeId() === id, [editor, id]);

    const childCount = useValue("childCount", () => editor.getSortedChildIdsForParent(id).length, [editor, id]);
