...
import type { CanvasSync } from "./canvas-sync";

// Copied from MIT-licensed inspirepan/canvas-cowork and adapted to also render
// Kronterm's semantic CanvasNode snapshots.
        ...
        <>
            ...
            >
                <div className="flex items-center gap-1.5 w-full">
                    {isFrame ? (
                        <button type="button" onClick={handleToggle} className="shrink-0 cursor-pointer p-0.5 -m-0.5">
                            {expanded ? (
            ...
            {isFrame && expanded && node.children.length > 0 && (
                <div className="border-l border-border" style={{ marginLeft: depth * 16 + 16 }}>
                    {node.children.map((child) => (
                        <TreeItem key={child.shapeId} node={child} depth={0} editor={editor} selectedId={selectedId} />
                    ))}
                </div>
            )}
        </>
...
function KrontermNodeIcon({ type }: { type: string }) {
    if (type === "frame") return <Folder className="h-3.5 w-3.5 shrink-0 text-muted-foreground" />;
    if (type === "image") return <Image className="h-3.5 w-3.5 shrink-0 text-muted-foreground" />;
    return <FileText className="h-3.5 w-3.5 shrink-0 text-muted-foreground" />;
}

function KrontermOutlineItems({
    editor,
    nodes,
    selectedId,
}: {
    ...
    if (nodes.length === 0) {
        return <div className="px-3 py-4 text-xs text-muted-foreground text-center">No items</div>;
    }
    return (
        <>
            {nodes.map((node) => {
                const selected = selectedId === node.shapeid;
                return (
                    <button
                        key={node.id}
                        type="button"
                        ...
                        className={`w-full flex items-center gap-1.5 rounded py-1 pr-2 pl-2 text-left text-xs hover:bg-accent/60 transition-colors ${
                            selected ? "bg-accent" : ""
                        }`}
                    >
                        <KrontermNodeIcon type={node.type} />
            ...
            {open && (
                <div className="min-w-48 max-w-80 max-h-[calc(100vh-400px)] bg-background/80 backdrop-blur border border-border rounded-lg shadow-sm flex flex-col overflow-hidden">
                    ...
                    <div className="flex-1 overflow-auto">
                        <div className="py-1">
                            {nodes ? (
                                <KrontermOutlineItems editor={editor} nodes={nodes} selectedId={selectedId} />
                            ) : tree.length === 0 ? (
