...
import { buildCacheBustedSrc, detectMovesEnhanced, ensureUniquePath } from "./canvas-sync-utils";
import type { CanvasFileEntry, CanvasFSEvent, CanvasSyncChange, ClientMessage } from "./protocol";
...
export class CanvasSync {
    ...
    async addImageFromBase64(base64: string, mimeType: string): Promise<{ shapeId: string; path: string }> {
        const ext =
            { "image/png": "png", "image/jpeg": "jpg", "image/webp": "webp", "image/gif": "gif" }[mimeType] ?? "png";
        const now = new Date();
    ...
    private async refreshImageAsset(shapeId: TLShapeId, relPath: string, mtimeMs?: number): Promise<void> {
        const shape = this.editor.getShape(shapeId as TLShapeId);
        if (!shape || shape.type !== "image") return;
        const assetId = (shape.props as { assetId?: string }).assetId;
        if (!assetId) return;
    ...
    handleFSChanges(changes: CanvasFSEvent[]): void {
        ...
        for (const change of changes) {
            ...
            if (change.action === "created" && !change.isDirectory && this.inferShapeType(change.path) === "image") {
        ...
        const hasDirChange = changes.some((c) => c.isDirectory && (c.action === "created" || c.action === "deleted"));
        if (hasDirChange) {
    ...
    private detectMoves(
        ...
        const eligibleDeletes = deletes.filter((del) => !del.isDirectory && this.fileToShape.has(del.path));
    ...
    private handleFSMoves(moves: { deleteEvent: CanvasFSEvent; createEvent: CanvasFSEvent }[]): void {
        for (const { deleteEvent, createEvent } of moves) {
            ...
            // Reparent and move
            this.applyRemote(() => {
                const targetParent = newParentId ? (newParentId as TLParentId) : this.editor.getCurrentPageId();
    ...
    private handleFSDeleted(event: CanvasFSEvent): void {
        ...
        if (event.isDirectory) {
            ...
            const childEntries = [...this.fileToShape.entries()].filter(([path]) => path.startsWith(prefix));
    ...
    private findOpenPosition(width = DEFAULT_WIDTH, height = 60): { x: number; y: number } {
        ...
        const visibleBounds = vp
            ? allBounds.filter((b) => b.x < vp.x + vp.w && b.x + b.w > vp.x && b.y < vp.y + vp.h && b.y + b.h > vp.y)
            : [];
    ...
    private organizeFrameChildren(frameId: TLShapeId): { w: number; h: number } | null {
        ...
        const frameH = Math.max(FRAME_HEADER_OFFSET + maxColHeight - SHAPE_SPACING + FRAME_INNER_PADDING, 120);
        this.editor.updateShape({
            id: frameId,
            type: "frame",
