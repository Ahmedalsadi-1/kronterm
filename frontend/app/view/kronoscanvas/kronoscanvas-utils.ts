...
export function summarizeCanvas(nodes: CanvasNode[], edges: CanvasEdge[]): string {
    ...
    // Identify \"Target\" nodes (like AI) and their sources
    ...
    for (const node of nodes) {
        ...
        const sourceTitles = sources.map(s => titles.get(s) || s).join(', ');
    ...
    if (edges.length > 0) {
        ...
        for (const edge of edges) {
            lines.push(`  ${titles.get(edge.fromnode) ?? edge.fromnode} -> ${titles.get(edge.tonode) ?? edge.tonode}${edge.label ? ` (${edge.label})` : ''}`);
        }
    }
