...
func getWaveAISettings(premium bool, rtInfo waveobj.ObjRTInfo, aiModeName string) (*uctypes.AIOptsType, error) {
	...
	if config.WaveAICloud && !telemetry.IsTelemetryEnabled() {
		return nil, fmt.Errorf("hosted AI cloud modes require telemetry to be enabled")
	}
...
type PostMessageRequest struct {
	...
	KronosAgent    string            `json:"kronosAgent,omitempty"`
	KronosProvider string            `json:"kronosProvider,omitempty"`
	KronosModel    string            `json:"kronosModel,omitempty"`
	KronosMode     string            `json:"kronosMode,omitempty"`
}
...
func WaveAIPostMessageHandler(w http.ResponseWriter, r *http.Request) {
	...
	if req.KronosAgent != "" {
		aiOpts.Agent = req.KronosAgent
	}
	if req.KronosProvider != "" && req.KronosModel != "" {
		aiOpts.Model = fmt.Sprintf("%s/%s", req.KronosProvider, req.KronosModel)
	}
	if req.KronosMode != "" {
		aiOpts.PermissionMode = req.KronosMode
	}
